...
```{r}
model1 <- analysis("num_of_interacted_variable_set_split.csv", "num_interacted_variable_set")
model2 <- analysis("num_of_interacted_visual_design_split.csv", "num_interacted_visual_design")
model3 <- analysis("num_of_exposed_visual_design_split.csv", "num_exposed_visual_design")
model4 <-analysis("num_of_exposed_variable_set_split.csv", "num_exposed_variable_set")
```

```{r interaction_analysis}
analysis = function(filename, col) {
  ...
  model <- brm(
    formula = as.formula(paste(col, "~ dataset * oracle * search * task")),
    chains = 2,
    cores = 2,
    iter = 2500,
    warmup = 1000,
  ...
  for (task_name in task_list) {
    ...
    fit_info <-
      draw_data_sub %>% group_by(search, oracle, dataset) %>% mean_qi(.value, .width = c(.95, .5))
    fit_info
    write.csv(
      fit_info,
      paste(
