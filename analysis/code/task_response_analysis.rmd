...
for (attr in analyses) {
  ...
  search_credible_intervals['task'] = attr
  ...
  oracle_credible_intervals['task'] = attr
  ...
  alg_credible_intervals['task'] = attr
...
search_draw_data_combined$task <- factor(search_draw_data_combined$task, levels=rev(analyses))
...
ggsave(file="search_rating_differences.png", plot=search_differences_plot, path = "../plots/posterior_plots/new")
write.csv(search_differences_intervals, paste("../plot_data/new/search_rating_differences.csv", sep=""), row.names = FALSE)

oracle_draw_data_combined$task <- factor(oracle_draw_data_combined$task, levels=rev(analyses))
...
ggsave(file="oracle_rating_differences.png", plot=oracle_differences_plot, path = "../plots/posterior_plots/new", width = 7, height = 7)
...
alg_draw_data_combined$task <- factor(alg_draw_data_combined$task, levels=rev(analyses))
alg_draw_data_combined$alg <- as.factor(alg_draw_data_combined$alg)
alg_differences_plot <- alg_draw_data_combined %>%
    ggplot(aes(x = diff_in_rating, y = task)) +
    ylab("Metric") +
...
ggsave(file="alg_rating_differences.png", plot=alg_differences_plot, path = "../plots/posterior_plots/new", width = 7, height = 7)
write.csv(alg_differences_intervals, paste("../plot_data/new/oracle_rating_differences.csv", sep=""), row.names = FALSE)
