...
generate_model_and_draws = function(data, attr, generate_eval_plots = FALSE) {
  ...
  ggsave(file=paste(filename, ".png", sep=""), plot=predictive_plot, path = "../plots/posterior_plots/new")
  ...
  ggsave(file=paste(filename, "_focused_open_split.png", sep=""), plot=predictive_plot_focused_open_split, path = "../plots/posterior_plots/new")
...
ggsave(file="oracle_rating_differences.png", plot=oracle_differences_plot, path = "../plots/posterior_plots/new")
write.csv(oracle_differences_intervals, paste("../plot_data/new/oracle_rating_differences.csv", sep=""), row.names = FALSE)



```
```{r focused_vs_open_ordinal}
library(tidyverse)
library(brms)
library(tidybayes)
library(gganimate)
...
generate_model_and_draws = function(data, attr, generate_eval_plots = FALSE) {
  filename = gsub("\\.", "_", attr)
  filename = paste0(filename,"focused_split")
  data$y = data[,attr]
  model <- brm(
    formula = bf(y ~ dataset * oracle * search * task_type),
    family = cumulative("probit"),
  ...
  ggsave(file=paste(filename, ".png", sep=""), plot=predictive_plot, path = "../plots/posterior_plots/focused_open_split/new")
  credible_intervals <- predictive_plot_data %>% group_by(search, oracle, task_type) %>% mean_qi(rating, .width = c(.95, .5))
  write.csv(credible_intervals, paste("../plot_data/focused_open_split/",filename,".csv", sep=""), row.names = FALSE, append=FALSE)
...
run_model_accuracy = function(data, task){
  ...
  prior_intercept = normal(.15, .6, autoscale = FALSE),
