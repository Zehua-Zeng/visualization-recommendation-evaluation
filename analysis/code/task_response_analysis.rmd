...
for (task_name in task_list) {
  ...
  plot <- ggplot(draw_data_sub, aes(
    ...
    labs(x = " Number Interacted Variable Set", y = "Condition") 
  plot
...
draw_data <- data %>%
  add_predicted_draws(model1,
                   seed = seed,
                   re_formula = NA) %>%
  group_by(search, oracle, task, .draw) %>%
  mutate(num_interacted_variable_set = weighted.mean(.prediction))



############################################################################

draw_data_prediction <- draw_data %>% subset(task=="3. Prediction")
prediction_plot <- draw_data_prediction %>%
  ggplot(aes(x = oracle, y = num_interacted_variable_set)) +
  stat_eye(.width = c(.95, .5)) +
  theme_minimal() +
  facet_grid(. ~ search)

prediction_intervals <- draw_data_prediction %>% group_by(oracle, search) %>% mean_qi(diff_in_time, .width = c(.95, .5))


diff_in_search_prediction <- draw_data %>% 
  group_by(search, task, .draw) %>%
...
diff_in_search_prediction %>%
      ggplot(aes(x = diff_in_num_interacted_variable_set, y = task)) +
      xlab(paste0("Expected Difference in Number Interacted Variable Set (",diff_in_search_prediction[1,'search'],")")) +
      ylab("Task")+
      stat_halfeye(.width = c(.95, .5)) +
      geom_vline(xintercept = 0, linetype = "longdash") +
      theme_minimal()

plot2 <- draw_data %>% subset(task=="4. Exploration") %>%
  ggplot(aes(x = oracle, y = num_interacted_variable_set)) +
  stat_eye(.width = c(.95, .5)) +
  theme_minimal() +
  facet_grid(. ~ search)

diff_in_oracle_prediction <- draw_data %>% 
  group_by(oracle, task, .draw) %>%
   summarize(num_interacted_variable_set = weighted.mean(.prediction)) %>%
   compare_levels(num_interacted_variable_set, by = oracle) %>%
   rename(diff_in_num_interacted_variable_set = num_interacted_variable_set) 

diff_in_oracle_prediction %>%
      ggplot(aes(x = diff_in_num_interacted_variable_set, y = task)) +
      xlab(paste0("Expected Difference in Number Interacted Variable Set (",diff_in_oracle_prediction[1,'oracle'],")")) +
      ylab("Task")+
      stat_halfeye(.width = c(.95, .5)) +
      geom_vline(xintercept = 0, linetype = "longdash") +
      theme_minimal()
