...
run_model_time = function(data, task, a_prior, a_sd, facet){
  ...
  generate_posterior_plot(m, 3, filename, "Time (Seconds)", facet)

}

# priors for completion time of each task were derived from a pilot study
run_model_time(subset(time_data, task == "1. Find Extremum"), "Find Extremum", 190.34, 96.57, FALSE)
run_model_time(subset(time_data, task == "2. Retrieve Value"), "Retrieve Value", 313.37, 88.50, FALSE)
run_model_time(subset(time_data, task == "3. Prediction"), "Prediction", 870.53, 657.68, FALSE)
run_model_time(subset(time_data, task == "4. Exploration"), "Exploration", 693.13, 193.30, FALSE)
...
run_model_time(subset(time_data, task_type == "Focused"), "Focused", 251.85, 62.9625, TRUE)
run_model_time(subset(time_data, task_type == "Open-Ended"), "Open-Ended", 781.83, 195.4575, TRUE)
