...
diff_in_search_prediction_plot <- diff_in_search_prediction %>%
    ...
    xlab(
      paste0(
        "Difference in average  interacted_var_sets (",
...
search_differences <- expected_diff_in_mean_plot(predictive_data_interacted_var_sets, "search", "",  "", NULL)
...
oracle_differences <- expected_diff_in_mean_plot(predictive_data_interacted_var_sets, "oracle", "",  "", NULL)
...
diff_in_group_prediction_plot <- diff_in_group_prediction %>%
    ...
    stat_halfeye(.width = c(.95, .5)) +
    geom_vline(xintercept = 0, linetype = "longdash") +
    theme_minimal() + scale_y_discrete(limits = rev(levels(diff_in_group_prediction$task))) + scale_alpha(guide = 'none')
...
diff_in_search_prediction_plot <- diff_in_search_prediction %>%
    ...
    xlab(
      paste0(
        "Difference in average  interacted_visual_design (",
        diff_in_search_prediction[1, 'search'],
        ")"
      )
    ) +
...
diff_in_search_prediction_plot <- diff_in_search_prediction %>%
    ...
    xlab(
      paste0(
        "Difference in average  exposed_variable_set (",
        diff_in_search_prediction[1, 'search'],
        ")"
      )
    ) +
...
diff_in_group_prediction_plot <- diff_in_group_prediction %>%
    ...
    xlab(
      paste0(
        "Difference in average  Exposed Variable Set (",
        diff_in_group_prediction[1, 'participant_group'],
        ")"
      )
    ) +
...
```
```{r}
diff_in_group_prediction_plot_split_by_dataset <- diff_in_group_prediction_plot + aes(fill = Dataset, alpha = 0.5) + scale_alpha(guide = 'none')
...
```{r}
diff_in_search_prediction_plot <- diff_in_search_prediction %>%
    ggplot(aes(x = diff, y = participant_group)) +
    xlab(
      paste0(
        "Difference in average exposed_visual_design (",
        diff_in_search_prediction[1, 'task'],
        ")"
      )
    ) +
    ylab("Task") +
