...
```{r}
search_difference_split_by_dataset <- search_differences$plot + aes(fill = dataset, alpha = 0.5)
search_difference_split_by_dataset

ggsave(
      file = "search_differences_split_by_dataset.png",
      plot = search_difference_split_by_dataset,
      path = "../plots/comparisons/num_interacted_variable_set"
    )
```
...
```
```{r}
oracle_differences_split_by_dataset <- oracle_differences$plot + aes(fill = dataset, alpha = 0.5)
oracle_differences_split_by_dataset

ggsave(
      file = "oracle_differences_split_by_dataset.png",
      plot = oracle_differences_split_by_dataset,
      path = "../plots/comparisons/num_interacted_variable_set"
    )
```
...
```
```{r}
group_differences_split_by_dataset <- participant_group_differences$plot + aes(fill = dataset, alpha = 0.5)
group_differences_split_by_dataset

ggsave(
      file = "group_differences_split_by_dataset.png",
      plot = group_differences_split_by_dataset,
      path = "../plots/comparisons/pariticpant_groups/num_interacted_variable_set"
    )
```
...
```
```{r}
search_differences_split_by_dataset <- search_differences$plot + aes(fill = dataset, alpha = 0.5)

search_differences_split_by_dataset

ggsave(
      file = "search_differences_split_by_dataset.png",
      plot = search_differences_split_by_dataset,
      path = "../plots/comparisons/num_interacted_visual_design"
    )
```
Difference in oracle
...
```
```{r}
group_differences_split_by_dataset <- participant_group_differences$plot + aes(fill = dataset, alpha = 0.5) + scale_alpha(guide = 'none')
group_differences_split_by_dataset
