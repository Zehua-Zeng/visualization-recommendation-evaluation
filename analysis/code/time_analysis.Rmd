...
The prior (normal(360.48, 224.40)) was derived from pilot studies.  It describes the distribution of time (in seconds) needed to complete any given task.  The lognormal family was selected to prevent our model from predicting completion times of less than zero seconds. 
...
time_data %>%
  ...
  add_predicted_draws(model, prediction = "completion_time", seed = seed) %>%
  ggplot(aes(x = completion_time)) +
  geom_density(fill = "gray", size = 0) +
  scale_y_continuous(NULL, breaks = NULL) +
  facet_grid(task~.) +
  labs(subtitle = "Prior predictive distribution for task completion time") +
